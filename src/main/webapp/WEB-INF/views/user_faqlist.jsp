...
<div class="faq-container">
	...
	<c:choose>
        <c:when test="${empty list}">
            <p class="no-result">해당하는 FAQ가 없습니다.</p>
        </c:when>
        <c:otherwise>
            <c:forEach var="faqDto" items="${list}">
                <div class="faq-item" data-id="${faqDto.fno}">
                    <span class="faq-title">${faqDto.f_question}</span>
                    <span class="toggle-icon">▼</span>
                </div>
                <div class="faq-content">
	                <pre>${faqDto.f_answer}</pre>
	                <span class="faq-date">
						<fmt:formatDate value="${faqDto.f_date}" pattern="yyyy.MM.dd" />
					</span>
                </div>
            </c:forEach>
        </c:otherwise>
...
<script>
	$(document).ready(function () {
	    $(".toggle-icon").click(function () {
	        var content = $(this).closest(".faq-item").next(".faq-content");
